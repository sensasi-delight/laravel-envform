...
- [ ] T001 Create workflow directory and skeleton file in `.github/workflows/quality-check.yml`
- [ ] T002 Configure basic workflow triggers and permissions in `.github/workflows/quality-check.yml`
...
- [ ] T003 Define strategy matrix for PHP (8.2, 8.3, 8.4) and stability (lowest, stable) in `.github/workflows/quality-check.yml`
- [ ] T004 Implement `shivammathur/setup-php` step with required extensions (pcov, mbstring) in `.github/workflows/quality-check.yml`
- [ ] T005 Configure Composer caching and dependency installation logic in `.github/workflows/quality-check.yml`
...
- [ ] T006 [US1] Implement Pint coding standard check (check-only mode) in `.github/workflows/quality-check.yml`
- [ ] T007 [US1] Implement PHPStan static analysis (Level 8) in `.github/workflows/quality-check.yml`
- [ ] T008 [US1] Implement PHPUnit test execution step in `.github/workflows/quality-check.yml`
- [ ] T009 [US1] Ensure workflow fails if any quality check fails in `.github/workflows/quality-check.yml`
...
- [ ] T010 [US2] Implement matrix stability logic (uses 'composer update' for prefer-lowest) in .github/workflows/quality-check.yml
- [ ] T011 [US2] Optimize standard build performance using 'composer install --prefer-dist --no-progress' for stable dependencies in .github/workflows/quality-check.yml
...
- [ ] T012 Implement code coverage reporting to GITHUB_STEP_SUMMARY using pcov in .github/workflows/quality-check.yml
- [ ] T013 Verify log accessibility and failure reporting (SC-004) in `.github/workflows/quality-check.yml`
- [ ] T014 Final validation of all Success Criteria (SC-001 to SC-002, SC-004)
- [ ] T015 Verify or document GitHub repository settings to enforce the "Tests" workflow as a "Required Status Check" (satisfies SC-003)
